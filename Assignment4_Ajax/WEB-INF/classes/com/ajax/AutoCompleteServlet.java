    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equals("lookup")) {
            ...
            if ((targetId != null) || products.containsKey(targetId.trim())) {
				...
				}else if((targetId.substring(0,2)).equals("el")){
					context.getRequestDispatcher("/ElectronicArts").forward(request, response);
				}else if((targetId.substring(0,2)).equals("ac")){
					context.getRequestDispatcher("/Activision").forward(request, response);
				}else if((targetId.substring(0,2)).equals("tt")){
					context.getRequestDispatcher("/TakeTwoInteractive").forward(request, response);
				}else if((targetId.substring(0,2)).equals("ac")){
